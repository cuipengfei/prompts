---
name: help-me-read
description: 当用户说“help me read”、“read this URL”、“read these links”、“帮我读”、“读一下这个网页”，或提供一个或多个 URL 并希望看到原文段落/段落组、必要时的逐段信达雅译文、轻量 commentary，以及文末 insights、key info、logical ordering、interpretation，而不是只看 AI 摘要时使用。
---

# Help Me Read

把一个或多个 URL 转成可快速阅读的阅读稿。核心目标不是替用户总结文章，而是保留原文肌理，让用户在原文、必要译文、轻量 commentary 与文末综合解读之间快速移动。阅读单元默认是自然段；相邻短段或同一小论证组块放在一起更好读时，可以合并成段落组。
...
| 文末综合 | 在逐段阅读稿之后追加 insights、key info、logical ordering 与 interpretation；它们基于已展示原文，不替代逐段阅读单元。 |
...
6. **追加文末综合解读**
   - 在所有逐段阅读单元之后，必须追加一个综合区；多个 URL 时，先给每个来源自己的综合区，必要时再给跨来源对照区。
   - 综合区必须从已经展示的原文、译文与 commentary 中抽取，不引入未来源支持的新事实；外部背景若确有必要，明确标注“补充背景/未由本文直接证明”。
   - 综合区包括四块：**Insights**、**Key Info**、**Logical Ordering**、**Interpretation**。
   - **Insights** 给 3-7 条非显然洞察：作者真正想推进的判断、张力、隐含前提、重要转折或可迁移观察。
   - **Key Info** 给 5-12 条关键信息：人物、组织、时间、数字、定义、事件、主张、条件、限制与例外；保留原文限定词，不把可能性写成确定性。
   - **Logical Ordering** 重建文章逻辑，而不是重排原文阅读单元：用编号说明“问题/背景 → 核心主张 → 证据 → 反驳/限制 → 结论/行动含义”等结构；若原文逻辑跳跃，直接指出跳跃点。
   - **Interpretation** 给克制解释：这篇文章在说什么、为什么这样说、用户读完应如何理解其意义与边界；区分“原文明确说了”“可合理推断”“本文没有证明”。
   - 综合区不重复整篇摘要；优先输出帮助用户复盘、记忆、判断的信息。
...
#### Key Info
- <5-12 条关键信息；保留限定词、数字与条件>

#### Logical Ordering
1. <原文逻辑步骤；不是重排逐段阅读单元>

#### Interpretation
<克制解释；区分原文明确内容、合理推断与未被证明部分>
```

多个 URL 时重复“来源 N”区块。不同 URL 的逐段阅读单元不要混写成一篇综合叙述；跨来源综合只在每个来源自己的文末综合之后单独追加。
...
- 文末综合可以重建逻辑顺序，但必须明确这是解读层；逐段阅读单元仍按原文顺序呈现。
- Interpretation 必须比原文更克制：只解释原文如何运作与可推出什么，不替作者补写未说出的结论。
...
- 不要省略文末综合区；用户明确只要原文/翻译时除外。
- 不要把解释、背景或个人判断塞进译文。
- 不要为了顺滑而改写作者立场。
- 不要重排段落；除非用户明确要 study guide 而不是 reading pass。
- 不要把 Logical Ordering 当作重新排序后的正文；它只是文末逻辑复盘。
- 不要在 Insights、Key Info 或 Interpretation 里加入本文没有支持的事实却不标注来源边界。
- 不要把不相邻段落合并成一个阅读单元。
- 不要用段落组掩盖删节；段落组不是摘要。
...
- 每个可用来源都按 URL 独立成块。
- 阅读单元卡片遵循 `原文 → 必要译文 → Commentary`。
- 若合并相邻段落，已标明段落范围，且原文信息与段落顺序完整保留。
- 原文语言不同于自动识别出的偏好输出语言时，已提供译文；相同时未重复翻译。
- Commentary 明显轻于原文与译文。
- 已在逐段阅读单元之后追加文末综合区，且包含 Insights、Key Info、Logical Ordering、Interpretation。
- 文末综合每条判断都能追溯到已展示的原文或已明确标注为补充背景/推断/未证明。
